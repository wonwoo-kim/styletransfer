...
def main(args):
    ...
    with tf.Session() as sess:
        ...
        picatensor = tf.summary.FileWriter("./tensorlog/",sess.graph)
        sess.run(init_op)
        vggnet.load_weights('libs/vgg16_weights.npz', sess)
        ...
        try:
            ...
            # in predict.py
            #final_saver.save(sess, './pbfolder/5_20_sec.ckpt')
            #tf.train.write_graph(sess.graph_def,".pbfolder",'plzplz.pb',as_text=False)
            #tf.train.write_graph(sess.graph_def,"./pbfolder",'plzplz.pbtxt',as_text=True)
            
            tf.io.write_graph(sess.graph, './pbfolder','5_20_update.pbtxt')
            tf.io.write_graph(sess.graph, './pbfolder', '5_20_update.pb',as_text=False)

            saver.save(sess,'TEST')
            freeze_graph.freeze_graph('./pbfolder/5_20_update.pbtxt',"",False,'TEST','Adam/update',"","",'frozen_update.pb',False,"")
            coord.request_stop()
